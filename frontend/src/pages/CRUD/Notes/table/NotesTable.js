...
import * as dataFormat from 'pages/CRUD/Notes/table/NotesDataFormatters';

import actions from 'actions/notes/notesListActions';
import actionsNotes from 'actions/notes/notesFormActions';
...
const NotesTable = () => {
  ...
  const count = useSelector((store) => store.notes.list.count);
  const modalOpen = useSelector((store) => store.notes.list.modalOpen);
  const rows = useSelector((store) => store.notes.list.rows);
  const idToDelete = useSelector((store) => store.notes.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsNotes.doUpdate(id, data));
  };
  ...
  return (
    <div>
      <Widget title={humanize('Notes')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/notes/new' className={classes.element}>
