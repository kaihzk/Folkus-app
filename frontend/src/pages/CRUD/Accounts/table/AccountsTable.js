...
import * as dataFormat from 'pages/CRUD/Accounts/table/AccountsDataFormatters';

import actions from 'actions/accounts/accountsListActions';
import actionsAccounts from 'actions/accounts/accountsFormActions';
...
const AccountsTable = () => {
  ...
  const count = useSelector((store) => store.accounts.list.count);
  const modalOpen = useSelector((store) => store.accounts.list.modalOpen);
  const rows = useSelector((store) => store.accounts.list.rows);
  const idToDelete = useSelector((store) => store.accounts.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsAccounts.doUpdate(id, data));
  };
  ...
  return (
    <div>
      <Widget title={humanize('Accounts')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/accounts/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Accounts'
                  : 'http://localhost:8080/api-docs/#/Accounts'
              }
            >
              API documentation for accounts
            </LinkMaterial>
          </div>
        }
      </Widget>
