...
import * as dataFormat from 'pages/CRUD/Connections/table/ConnectionsDataFormatters';

import actions from 'actions/connections/connectionsListActions';
import actionsConnections from 'actions/connections/connectionsFormActions';
...
const ConnectionsTable = () => {
  ...
  const count = useSelector((store) => store.connections.list.count);
  const modalOpen = useSelector((store) => store.connections.list.modalOpen);
  const rows = useSelector((store) => store.connections.list.rows);
  const idToDelete = useSelector((store) => store.connections.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsConnections.doUpdate(id, data));
  };
  ...
  const sendConnectionsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('connections/bulk-import', data, {
        headers: {
  ...
  const getConnectionsCSV = async () => {
    const response = await axios({
      url: '/connections?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'connectionsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Connections')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/connections/new' className={classes.element}>
