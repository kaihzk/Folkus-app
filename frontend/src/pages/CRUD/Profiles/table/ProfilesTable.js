...
import * as dataFormat from 'pages/CRUD/Profiles/table/ProfilesDataFormatters';

import actions from 'actions/profiles/profilesListActions';
import actionsProfiles from 'actions/profiles/profilesFormActions';
...
const ProfilesTable = () => {
  ...
  const count = useSelector((store) => store.profiles.list.count);
  const modalOpen = useSelector((store) => store.profiles.list.modalOpen);
  const rows = useSelector((store) => store.profiles.list.rows);
  const idToDelete = useSelector((store) => store.profiles.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsProfiles.doUpdate(id, data));
  };
  ...
  return (
    <div>
      <Widget title={humanize('Profiles')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/profiles/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Profiles'
                  : 'http://localhost:8080/api-docs/#/Profiles'
              }
            >
              API documentation for profiles
            </LinkMaterial>
          </div>
        }
      </Widget>
